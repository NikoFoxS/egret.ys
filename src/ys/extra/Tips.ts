module ys {
	...
	export class Loading extends egret.DisplayObjectContainer {
		constructor() {
			...
			const ctn = new ys.Container() //ys.newContainer();
			this.addChild(ctn);

			const width = 200;
			const height = 200;
			const bg = new ys.Shape();//ys.newRect(width, height, 0x000000, 40, ctn);
			bg.drawRec(width, height, 0x000000, 40);
			ctn.addChild(bg);
			bg.alpha = 0.7;
	...
	export class Modal extends egret.DisplayObjectContainer {
		constructor({msg, confirm = '', cancel = '', fontSize = 50, txtWidth = 500}) {
			...
			const bg = new ys.Shape();//ys.newRect(bgw, bgh, 0xffffff, 40);
			bg.drawRec(bgw, bgh, 0xffffff, 40);
			ctn.addChild(bg);
	...
	export class Toast extends egret.DisplayObjectContainer {
		constructor({msg, color = 0xffffff, bgColor = 0x000000, icon = '', bgAlpha = 0.8, padding = 30}) {
			...
			let icoH = 0;
			if (icon != '') {
				var ico = new ys.Bitmap();
				ico.create(icon);
				this.addChild(ico);// ys.newBitmap(icon, this);
			...
			const bg = new ys.Shape();
			bg.drawRec(t.width + padding2, t.height + padding2 + icoH, bgColor, padding);
			bg.alpha = bgAlpha;
