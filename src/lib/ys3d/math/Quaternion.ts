module ys3d {
	export class Quaternion extends Array {
		...
		public static fromEuler(out: Quaternion, euler: Euler, order = 'YXZ') {
			//这里改成弧度
			let rad = Math.PI/180;
			let sx = Math.sin(euler[0]*rad * 0.5);
			let cx = Math.cos(euler[0]*rad * 0.5);
			let sy = Math.sin(euler[1]*rad * 0.5);
			let cy = Math.cos(euler[1]*rad * 0.5);
			let sz = Math.sin(euler[2]*rad * 0.5);
			let cz = Math.cos(euler[2]*rad * 0.5);
